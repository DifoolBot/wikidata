...
| `CORRECT_AS_OF_NOW` | every id in the old cluster is now a non-deprecated id on this item — VIAF resolved it | **un-deprecate** (normal rank, drop the `P2241` qualifier) + stamp. If an id *also* lands in a foreign rival cluster, the item is **additionally flagged for review** (the rival may mean the item mixes two people) — un-deprecate *and* review |
...
| `NEW_CLUSTER_CONFLATED` | the cluster the ids would be adopted into carries an authority id the item deprecated as `Q35773207` (refers to different person) — VIAF conflated a foreign id into it, so it isn't adopted | review list |
| `DUPLICATE_RANK` | the same `P214` value is on the item at **both** a non-deprecated rank and deprecated-for-conflation — a contradiction (caught before any VIAF query, so it's free) | review list |
