...
class LocPage(authdata.AuthPage):
    ...
    def __str__(self):
        return """
          ...
          """.format(loc_id=self.id, not_found=self.not_found, is_redirect=self.is_redirect,
                     name=self.name.names_en(), language=self.languages,
                     given_name_en=self.name.given_name_en, family_name_en=self.name.family_name_en,
                     birth_date=self.birth_date, death_date=self.death_date,
                     )

    def query(self):
        url = "https://id.loc.gov/authorities/names/{id}.json".format(
            id=self.id
        )
    ...
    def is_family_name_first_language(self, user_languages):
        family_name_first_languages = {
            'jpn', 'kor', 'chi', 'zho', 'vie', 'khm'}
        return any(lang in family_name_first_languages for lang in user_languages)

    def name_order(self):
        print(f'language: {self.languages}')
        family_name_first = 0
        family_name_last = 0
        ...
        if element_list:
            for p in data:
                if p["@id"] in element_list:
                    name_part = {
                        'type': p['@type'][0],
                        'value': p['http://www.loc.gov/mads/rdf/v1#elementValue'][0]["@value"]
                    }
        ...
        for p in data:
            if p["@id"] == "http://id.loc.gov/authorities/names/" + self.init_id:
                if 'http://www.w3.org/2004/02/skos/core#prefLabel' in p:
                    pref_label = p["http://www.w3.org/2004/02/skos/core#prefLabel"][0]["@value"]
                    if name_parts != []:
                        print(f'pref_label: {pref_label}')
                        print(f'name parts: {name_parts}')
                        for name_part in name_parts:
                            if name_part['type'] != 'http://www.loc.gov/mads/rdf/v1#FullNameElement':
                                part = name_part['value']
                                if part not in pref_label:
                                    raise ValueError(
                                        f'{part} not in {pref_label}')
                ...
                if 'http://www.loc.gov/mads/rdf/v1#birthDate' in p:
                    self.birth_date = p['http://www.loc.gov/mads/rdf/v1#birthDate'][0]['@value']
                if 'http://www.loc.gov/mads/rdf/v1#deathDate' in p:
                    self.death_date = p['http://www.loc.gov/mads/rdf/v1#deathDate'][0]['@value']
