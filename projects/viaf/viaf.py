...
AUTHORITY_SOURCE_CODE_NATIONAL_AND_UNIVERSITY_LIBRARY_IN_ZAGREB = "NSK"
...
class ZagrebAuthoritySource(AuthoritySource):
    def getCode(self):
        return AUTHORITY_SOURCE_CODE_NATIONAL_AND_UNIVERSITY_LIBRARY_IN_ZAGREB
...
class ViafBot:
    ...
    def query_viaf(self, local_auth_id):
        try:
            url = "https://viaf.org/viaf/sourceID/{code}|{local_auth_id}/justlinks.json".format(
                code=self.auth_src.getCode(), local_auth_id=local_auth_id
            )
            data = requests.get(url).json()
            return data
        except json.JSONDecodeError as e:
            print(
                "{desc} ID {local_auth_id} returns an error: {error}".format(
                    desc=self.auth_src.getDescription(), local_auth_id=local_auth_id, error=e
                )
            )
            return []
    ...
    def check_viaf_justlinks(self, data, qid, local_auth_id):
        ...
        if code in data:
            if local_auth_id in data[code]:
                ...
                print(
                    "viaf id {viaf_id} of {qid} has another {desc} link".format(
                        viaf_id=viaf_id, qid=qid, desc=self.auth_src.getDescription()
                    )
                )
                res["has_error"] = True
            ...
            if len(data[code]) > 1:
                print(
                    "viaf id {viaf_id} of {qid} has multiple {desc} links".format(
                        viaf_id=viaf_id, qid=qid, desc=self.auth_src.getDescription()
                    )
                )
                res["has_error"] = True
            ...
            if qid in data[AUTHORITY_SOURCE_CODE_WIKIDATA]:
                ...
                print(
                    "viaf id {viaf_id} of {qid} has another wikidata link".format(
                        viaf_id=viaf_id, qid=qid
                    )
                )
                res["has_error"] = True

            if len(data[AUTHORITY_SOURCE_CODE_WIKIDATA]) > 1:
                print(
                    "viaf id {viaf_id} of {qid} has multiple wikidata links".format(
                        viaf_id=viaf_id, qid=qid
                    )
                )
                res["has_error"] = True
    ...
    def change_wikidata(self, qid, local_auth_id, viaf_id) -> None:
        ...
        if self.auth_src.getPID() not in existing_claims:
            print("Skipping {qid} because it has no {code} PID".format(
                qid=qid, code=self.auth_src.getPID()))
            return

        found = False
        for claim in existing_claims[self.auth_src.getPID()]:
            id = claim.getTarget()
            if id == local_auth_id:
                if claim.getRank() == "deprecated":
                    print("Skipping {qid} because the {code} {local_auth_id} is deprecated".format(
                        qid=qid, code=self.auth_src.getPID(), local_auth_id=local_auth_id))
                    return
                found = True
                break

        if not found:
            print("Skipping {qid} because it has no {code} {local_auth_id}".format(
                qid=qid, code=self.auth_src.getPID(), local_auth_id=local_auth_id))
            return
        ...
        try:
            ...
            item.addClaim(
                claim,
                summary="Adding VIAF ID based on {desc} ID".format(
                    desc=self.auth_src.getDescription()
                ),
            )

            claim.addSources(
                self.create_viaf_ref(viaf_id), summary="Adding VIAF reference"
            )
    ...
    def get_duplicates_qids(self, viaf_id):
        res = []
        query = 'SELECT DISTINCT ?item WHERE {{ ?item p:P214 ?statement0. ?statement0 (ps:P214) "{viaf_id}".}} LIMIT 5'.format(
            viaf_id=viaf_id
        )
    ...
    def iterate_viaf(self, qid, local_auth_id) -> None:
        ...
        res = self.check_viaf_justlinks(
            self.query_viaf(local_auth_id), qid, local_auth_id
        )
        if res is None:
            return
        ...
        if not has_local_auth_id:
            print(
                "{viaf_id} has no {desc} link".format(
                    viaf_id=viaf_id, desc=self.auth_src.getDescription()
                )
            )
            return
        ...
        if duplicate_qids != []:
            print("{qid} has duplicates: {dupl}".format(
                qid=qid, dupl=duplicate_qids))
                ...
                self.duplicates[qid].append(
                    {
                        ...
                        "auth_code": self.auth_src.getCode(),
            ...
            self.change_wikidata(qid, local_auth_id, viaf_id)
...
def main() -> None:
    ...
    bot = ViafBot(ZagrebAuthoritySource())
    bot.test = True
    bot.run()
