...
class IdrefPage(authdata.AuthPage):
    ...
    def __str__(self):
        return """
          ...
          """.format(idref=self.id, not_found=self.not_found, is_redirect=self.is_redirect,
                     name=self.name.names_en(), gender=self.sex, citizenship=self.citizenship,
                     given_name=self.name.given_name, family_name=self.name.family_name,
                     given_name_en=self.name.given_name_en, family_name_en=self.name.family_name_en,
                     )

    def query(self):
        url = "https://www.idref.fr/{id}.rdf".format(
            id=self.id
        )
    ...
    def name_order(self):
        ...
        id = self.citizenship.replace(
            'http://sws.geonames.org', '').replace('/', '')
        if id in lst:
    ...
    def run(self):
        ...
        if person is not None:
            ...
            self.name = name.Name(name_en=pref_label, given_name=given_name, family_name=family_name)

            element = person.find('dbpedia:citizenship', ns)
            if element is not None:
                self.citizenship = element.attrib['{http://www.w3.org/1999/02/22-rdf-syntax-ns#}resource']
