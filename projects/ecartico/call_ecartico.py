...
from ecartico.interface_ecartico_data import IEcartico, IEcarticoData
...
class EcarticoBot(IEcartico):
    def __init__(
        self,
        generator,
        cached_data: IEcarticoData,
    ...
    def examine_item(self):
        ...
        if not self.ignore_error_list and self.cached_data.has_error(
            self.structure.qid
        ):
            print(f"{self.structure.qid}: skipped, in error list")
            return

        if not self.ignore_done_list and self.cached_data.has_done(self.structure.qid):
        ...
        if not self.item.exists():
            ...
            self.cached_data.add_done(self.structure.qid)
            return
        ...
        try:
            ...
            if self.structure.qid != "Q13406268":
                self.cached_data.add_person_qid(
            ...
            print("--")
            self.structure.apply(self, wikidata)

            self.cached_data.add_done(self.structure.qid)
        except RuntimeError as e:
            print(f"{self.structure.qid}: Runtime error: {e}")
            self.cached_data.add_error(self.structure.qid, e.__repr__())
...
def try_one(data: IEcarticoData, qid: str, ecartico_id: Optional[str] = None):
    bot = EcarticoBot(None, data, ignore_done_list=True, ignore_error_list=True)
    bot.examine(qid, ecartico_id=ecartico_id)


def iterate_all(data: IEcarticoData):
    ...
    bot = EcarticoBot(generator, data)
...
def sync_occupation(data: IEcarticoData):
    ...
    with open(
        ...
        for line in file:
            ...
            id = match.group(1)
            code = match.group(2)
            current = data.get_occupation_qid(id)
            if current == LEEG:
                if code == SKIP:
                    data.add_occupation_qid(id, "", "SKIP")
                    print(f"{id} -> SKIP")
                elif code.startswith("Q"):
                    data.add_occupation_qid(id, "", code)
                    print(f"{id} -> {code}")


def sync_source(data: IEcarticoData):
    print("sync sources")
    # Open the file in read mode
    with open(
        ...
        for line in file:
            ...
            id = match.group(1)
            code = match.group(2)
            current = data.get_source_qid(id)
            if current == LEEG:
                if code == SKIP:
                    data.add_source_qid(id, "", "SKIP")
                    print(f"{id} -> SKIP")
                elif code.startswith("Q"):
                    data.add_source_qid(id, "", code)
                    print(f"{id} -> {code}")


def sync_place(data: IEcarticoData):
    print("sync places")
    # Open the file in read mode
    with open(
        ...
        for line in file:
            ...
            id = match.group(1)
            code = match.group(2)
            current = data.get_place_qid(id)
            if current == LEEG:
                if code == SKIP:
                    data.add_place_qid(id, "", "SKIP")
                    print(f"{id} -> SKIP")
                elif code.startswith("Q"):
                    data.add_place_qid(id, "", code)
                    print(f"{id} -> {code}")
