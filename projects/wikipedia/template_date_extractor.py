...
class LanguageConfig:
    ...
    def is_known_template(self, name: str) -> bool:
        ...
        return name in self.template_map or name in self.date_template_map or name in self.ignore_templates
    ...
    def normalize_date_str(self, date_str: str) -> str:
        ...
        if self.month_map:
            ...
            for local_name, eng_name in sorted(self.month_map.items(), key=lambda x: -len(x[0])):
...
class TemplateDateExtractor:
    ...
    """
    def __init__(self, tpl_cfg: dict, tpl, lang_config: LanguageConfig, country_config: CountryConfig):
            ...
            for key, meaning in self.param_names.items():
                ...
                for inner_tpl in val.filter_templates():
                    ...
                    for tpl_cfg in self.lang_config.get_tpl_cfg(name):
                        extractor = TemplateDateExtractor(tpl_cfg, inner_tpl, self.lang_config, self.country_config)
                ...
                if dates:
                    ...
                    if names:
                        raise RuntimeError(f"Templates '{names}' in '{tpl.name}' did not yield any dates.")
        ...
        julian_val = self.values.get('julian', '').strip().lower() if 'julian' in self.values else ''
    ...
    def __parse_date_string(self, date_str: str, dayfirst: bool = None, calendar_url: str = URL_UNSPECIFIED_CALENDAR, typ: str = None) -> bool:
        ...
        try:
            ...
            tup = (typ, y if y else None, m if m else None, d if d else None, calendar_url)
    ...
    def _parse_date_string(self, date_str: str, dayfirst: bool = None, calendar_url: str = URL_UNSPECIFIED_CALENDAR, typ: str = None) -> bool:
        ...
        if not result:
            ...
            result = self.__parse_date_string(cleaned_date_str, dayfirst, calendar_url, typ)
        if result:
    ...
    def _parse_components(self, day, month, year, calendar_url: str, typ: str = None):
        ...
        if year:
            ...
            self._parse_date_string(date_str, dayfirst=True, calendar_url=calendar_url, typ=typ)
    ...
    def get_all_dates(self) -> list:
        ...
        for typ in ['birth', 'death', '']:
            has_julian = any(self.get_value(typ, k) for k in ['year julian', 'month julian', 'day julian'])
            ...
            if g_year:
                ...
                self._parse_components(g_day, g_month, g_year, cal_model, typ=typ if typ else None)
            if has_julian:
                ...
                self._parse_components(j_day, j_month, j_year, URL_PROLEPTIC_JULIAN_CALENDAR, typ=typ if typ else None)
            ...
            if date:
                ...
                self._parse_components(day, month, year, self.default_calendar_url, typ=typ if typ else None)
